...
function animateCompanyEnter(evt, destUrl){
  ...
  function applyNewContent(html, title){
    ...
    // row's <img>, not from any wrapper around it. If the destination's
    // wrapping div has any padding or the image isn't perfectly flush
    // with its edges, measuring the div instead of the image produces a
    // small but consistent offset between clone and real logo -- exactly
    // the kind of diagonal "double" that showed up after settling.
    ...
    // loaded (e.g. on a hard refresh, with nothing cached). Only real
    // <img> elements have a meaningful .complete/load -- the AI Native
    // icon is a styled span, which would never fire load and hang this
    // forever, so only wait when we actually have an image.
