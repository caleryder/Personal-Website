...
function animateCompanyEnter(evt, destUrl){
  ...
  function withRealContent(html, title){
    ...
    // Hide the freshly-inserted content immediately, in the same
    // synchronous block as the innerHTML swap above -- no yield to the
    // browser in between, so the real page (in its natural, final
    // position) is never actually painted to screen before we push it
    // out of view. Every wordmark image now has explicit width/height
    // (fixed separately), so its layout position is already correct
    // here without needing to wait for the image bytes to load.
