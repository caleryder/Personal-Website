...
/* ---------- company row click: clone rises directly to the real
   destination logo's position ----------
   Earlier versions estimated the destination position with a probe
   element before the real page content existed, started the clone
   moving toward that estimate immediately, then corrected it once the
   real content loaded. That correction step -- however small -- was
   itself a visible position change, which showed up as a brief offset
   "double" right before landing. This version never guesses: it waits
   until the real destination content is loaded and measurable, then
   moves the clone in one motion straight to the real logo's position.
   The trade-off is a brief pause after the click (while the real page
   content loads) before any motion starts, instead of motion starting
   immediately toward a guess. */
function animateCompanyEnter(evt, destUrl){
  ...
  // The clone sits still here at the source position until the real
  // destination content is ready to measure -- see withRealContent below.

  function withRealContent(html, title){
    content.innerHTML = html;
    if(title) document.title = title;

    const realMark = content.querySelector('.company-wordmark');
    // Measure the actual image/icon element itself, not its wrapping div --
    // the source measurement (srcRect, above) is taken directly from the
    // row's <img>, not from any wrapper around it. Measuring the div
    // instead of the image, if the div has any padding, produces a small
    // but consistent offset between clone and real logo.
    const realMarkImg = realMark ? (realMark.querySelector('img, .ai-native-icon-mark') || realMark) : null;

    function moveToRealPosition(){
      const destRect = realMarkImg ? realMarkImg.getBoundingClientRect() : srcRect;
      const upDistance = srcRect.top - destRect.top;

      content.style.transition = 'none';
      content.style.transform = `translateY(${upDistance}px)`;
      void content.offsetHeight; // force reflow so the transition below actually animates

      requestAnimationFrame(() => {
        // The content block and the clone move together, in sync, so the
        // logo and the page arrive together -- one motion, straight to
        // the real (already-measured) destination, in both directions
        // at once, no separate horizontal-then-vertical staging.
        content.style.transition = `transform ${stagger}ms ease`;
        content.style.transform = 'translateY(0)';
        clone.style.transition = `left ${stagger}ms ease, top ${stagger}ms ease`;
    ...
    // gives a slightly wrong target. Wait for the image first when it
    // isn't already loaded (e.g. on a hard refresh, with nothing cached).
    // Only real <img> elements have a meaningful .complete/load -- the AI
    // Native icon is a styled span, which would never fire load and hang
    // this forever, so only wait when we actually have an image.
    const isRealImg = realMarkImg && realMarkImg.tagName === 'IMG';
    if(isRealImg && !realMarkImg.complete){
      realMarkImg.addEventListener('load', moveToRealPosition, {once:true});
      realMarkImg.addEventListener('error', moveToRealPosition, {once:true});
    } else {
      moveToRealPosition();
    }
  }

  // In the interactive preview, page content is already in memory, so
  // there's no fetch delay at all.
  if(typeof ROUTES === 'object' && ROUTES[destUrl]){
    withRealContent(ROUTES[destUrl].html, ROUTES[destUrl].title);
    return;
  }
  ...
  fetch(destUrl).then(r => {
    ...
    withRealContent(newContent ? newContent.innerHTML : '', doc.title);
  }).catch(() => {
