...
function initScrollReveal(){
  const allTargets = document.querySelectorAll('.cs-headline, .page-title, .cs-lede, .lede, .cs-subhead, .cs-section p, .cs-section-label, .section-title, .stat-row, .pillar-full h3, .pillar-body, .pillar-detail, .pillar-quote p, .pillar-quote-attribution, .body-copy p, .leadership-block p');
  // The very first headline and lede (above the fold, visible the instant
  // the page loads) stay immediately visible -- same reasoning as the
  // primary hero image being excluded below: above-the-fold content must
  // not depend on a script finishing in time to be seen. Everything else
  // (subheads, section labels, body paragraphs, stats, pillar content)
  // fades in as it's scrolled to.
  const firstHeadline = document.querySelector('.cs-headline, .page-title');
  const firstLede = document.querySelector('.cs-lede, .lede');
  let targets = Array.from(allTargets).filter(el => el !== firstHeadline && el !== firstLede);
