...
function animateCompanyEnter(evt, destUrl){
  ...
  function applyNewContent(html, title){
    ...
    // measured before the real page content existed. That's fine for
    // starting the horizontal slide early (during the fetch), but the
    // real destination wordmark -- now that it actually exists in the
    // DOM -- can land a few pixels off from that estimate (e.g. if the
    // probe's empty box doesn't match the real logo's own sizing). Left
    // uncorrected, the clone rises toward the old estimate while the
    // real logo renders at its own true spot, producing a visible
    // offset "double" logo during the rise that only resolves once the
    // clone is removed. Re-measure the real thing and use that instead.
    ...
    requestAnimationFrame(() => {
      ...
      // in sync, so the logo and the page arrive together. Snap the
      // clone's horizontal position to the real mark too, in case that
      // ever drifts slightly from the earlier probe-based estimate.
