...
function animateCompanyEnter(evt, destUrl){
  ...
  const stagger = 450; // matches the pace already used elsewhere on the site
  ...
  requestAnimationFrame(() => {
    // Stage 1: horizontal move + resize to the destination's width/height.
    clone.style.transition = `left ${stagger}ms ease, width ${stagger}ms ease, height ${stagger}ms ease`;
  ...
  setTimeout(() => {
    // Stage 2: vertical move, only after stage 1 has finished.
    clone.style.transition = `top ${stagger}ms ease`;
    clone.style.top = destRect.top + 'px';
  }, stagger);

  setTimeout(() => {
    // In the interactive preview (a single file, no real navigation),
    // hand off to its content-swap router instead of a real page load.
    if(typeof renderRoute === 'function'){
      const hashIdx = destUrl.indexOf('#');
      const path = hashIdx >= 0 ? destUrl.slice(0, hashIdx) : destUrl;
      const frag = hashIdx >= 0 ? destUrl.slice(hashIdx + 1) : null;
      renderRoute(path, frag);
      clone.remove();
    } else {
